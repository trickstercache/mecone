...
We will run and publish quarterly 'State of the Industry' reports to this space, using the `industry-test` catalog. As Mecone evolves to include more exhaustive test cases, scores will change over time. For verification, run this report locally with `make industry-wide` and the results will be published to `catalogs/industry-wide/reports`.
