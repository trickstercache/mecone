...
var directOutcomes = map[string]results.Outcome{
	...
	"freshness-must-revalidate-defeats-stale-if-error":      results.Pass,
	"freshness-s-maxage-defeats-stale-while-revalidate":     results.Pass,
	...
	"invalidation-auth-write-removes-shared-response":       results.Pass,
	"invalidation-removes-every-vary-variant":               results.Pass,
	...
	"ranges-stored-if-range-mismatch-needs-full-response":   results.Pass,
	...
	"stale-background-refresh-keeps-active-validator":       results.Skipped,
	...
	"storage-authorized-negative-response-is-not-shared":    results.Pass,
	...
	"storage-replacement-renews-authorized-shareability":    results.Pass,
	...
	"vary-concurrent-cold-misses-remain-isolated":           results.Pass,
